...
def rendering(new_dict, new_list=None, tab=''):
    ...
    for key in new_dict:
        if key[0] == 'added' and isinstance(new_dict[key], dict):
            new_list.append(
                    f'{tab}{ADD}{key[1]}: {START}\n'
                    f'{stringify(new_dict[key], tab)}\n'
                    f'{TAB}{tab}{END}'
                    )
        elif key[0] == 'added':
        ...
        if key[0] == 'changed' and isinstance(new_dict[key], dict):
            new_list.append(
                    f'{tab}{TAB}{key[1]}: {START}\n'
                    f'{tab}{DEL}{key[1]}: {truefy(new_dict[key][0])}\n'
                    f'{tab}{ADD}{key[1]}: {truefy(new_dict[key][1])}\n'
                    f'{TAB}{tab}{END}'
                    )
        elif key[0] == 'changed':
            new_list.append(
